...
class Style:
    ...
    #: レーン図で、レーン名を出す帯の高さ。どちらの列が誰なのかは、図の中で
    #: いちばん先に読まれる必要があるので、箱と同じ高さは取らない(名前だけ)。
    diagram_lane_header: float = 0.46
    #: レーン図で、左右のレーンのあいだ。縦線を引き、担当が変わる矢印がここを
    #: またぐ。狭いと、またいだことが見えない。
    diagram_lane_gutter: float = 1.0
    #: レーン図の、行と行のあいだ(矢印の置き場所)。流れ図より広く取る。
    #: レーンをまたぐ矢印は横長になるので、細いと線に見える。
    diagram_lane_gap: float = 0.38
    #: レーン図で、戻りの矢印と札を置く帯の幅の下限と上限。札はこの帯の中で
    #: 折り返す(戻りの理由は文になりがちで、1 行で置くと図が横に伸びきる)。
    #: 実際の幅は札の長さから決める —— 狭いと語の途中で改行が入り、
    #: 広いと左が空いたまま図全体が縮む。
    diagram_lane_return_min: float = 1.5
    diagram_lane_return_max: float = 2.8
    #: 戻りの帯のうち、札が使える割合(残りは矢印の縦棒の置き場所)。
