...
どれを標準にするかを決めるための聴き比べは [note2slides-voices](#ナレーション音声の聴き比べnote2slides-voices) で作れます。
...
**聞く順番は 2 段階です。** まず `preview.wav`(サンプル記事・候補 6 つで約 4 分)で
候補の冒頭が続けて流れるので、声の違いはここで分かります。
そのうえで気になった候補の `continuous.wav`(同じく 4〜6 分)を通して聞きます。
...
長時間のeラーニングという用途で見ると、選ぶ観点は 3 つです。

1. 30 分聞いても疲れないか(基本周波数が高すぎない・抑揚が過剰でない)
2. 声ではなく内容に注意が向くか(キャラクター性・演技の強さが弱い)
3. 教材として不自然でないか(語尾が甘えない・断定が弱くならない)

VOICEVOX の話者の多くは会話や配信のキャラクターとして作られていて、長い説明文を読ませる
想定ではないため、この 3 つで見るとほとんどが外れます。残るのは「読み上げ用のスタイルを持つ話者」と
「抑揚の振れ幅が小さい落ち着いた話者」で、そこから声域の違う 4 話者を選びました。
現行標準については、話者を変えずに設定だけで改善できるかも比べたいので、設定違いも入れています。

| 候補 | 話者/スタイル | 位置づけ |
| --- | --- | --- |
| `no7-announce` | No.7 / アナウンス | 現行の標準(比較の基準) |
| `no7-calm` | No.7 / アナウンス | 同じ声を、速さ・抑揚・間で落ち着かせたもの |
| `no7-storytelling` | No.7 / 読み聞かせ | 同じ声の、聞かせるための別スタイル |
| `aoyama-normal` | 青山龍星 / ノーマル | 低音の男性 |
| `kurono-normal` | 玄野武宏 / ノーマル | 中音域の男性(既存実装が 2 番目に選ぶ声) |
| `meimei-normal` | 冥鳴ひまり / ノーマル | 低めの女性 |
...
| 変えるもの | 場所 |
| --- | --- |
| 標準の話者/スタイル | `src/note2slides/voicevox.py` の `PREFERRED_STYLES` の先頭 |
| 標準の速さ・抑揚・間 | `src/note2slides/audio_cli.py` の `--speed` / `--intonation` / `--sentence-pause` / `--line-pause` の既定値 |

1 回だけ試すなら、`--voice` で指定できます(動画生成でも同じです)。
...
| 標準にする声を決める材料 | `voice_compare.py` の `CANDIDATES`(聴き比べ用の音声を作る) |
